...
volatile u8 set_predict = 1;            // 默认开启预测 
volatile u8 display_flag = 0;           // 显示标志
volatile u8 predict_flag = 0;           // 预测标志
volatile u8 alarm_peoceed = 0;       // 警报处理
...
void TIM3_IRQHandler(void)
{
    if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)
    {
		...
		LCD_ShowString(30, 280, 200, 16, 16, "FRAMES tim3:tim2");
		LCD_ShowNum(30, 300, tim3_frames, 5, 16);
		LCD_ShowNum(100, 300, tim2_frames, 5, 16);
...
int main(void)
{
    ...
    while(1)
    {
		...
		if(key == KEY1_PRES)
		{
			set_predict = !set_predict; // 设置预测标志
			if(set_predict)
				LCD_ShowString(30, 170, 200, 16, 16, "PREDICT ON ");
			else
