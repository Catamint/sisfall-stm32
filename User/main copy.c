...
int get_features(float window[][6], int window_index, float* features)
{
	...
	for (p = 0; p < WINDOW_SIZE; p++)
	{
		...
		// if (i < 0 || i >= WINDOW_SIZE) continue; // 防止越界
		float* window_ptr = window[i];
...
volatile float prediction;              // 预测值    
volatile u8 report = 1;                 // 默认开启上报
volatile u8 display_flag = 0;           // 显示标志
volatile u8 predict_flag = 0;           // 预测标志
volatile float window[WINDOW_SIZE][6] = {0};
volatile int window_index = 0;
volatile float features[14] = {0};      // 特征数组
volatile Model svm_model;               // SVM模型
...
void TIM2_IRQHandler(void)
{
    if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET)
    {
        ...
        if(mpu_dmp_get_data(&pitch, &roll, &yaw) == 0)
        {
            ...
            // 存储到时间窗口
            window_index = put_window((float(*)[6])window, window_index, aacx, aacy, aacz, gyrox, gyroy, gyroz);
...
void TIM3_IRQHandler(void)
{
    if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)
    {
        ...
        // 特征提取
        get_features((float(*)[6])window, window_index, (float*)features);
        
        // 设置预测标志，主循环中处理预测和警报判断
        predict_flag = 1;
...
int main(void)
{
    ...
    LCD_ShowString(30, 200, 200, 16, 16, "  ");
    ...
    while(1)
    {
        ...
        if(predict_flag)
        {
            ...
            // 计算触发警报的阈值
            alarm_threshold = (short)(abs(aacx)/100 + abs(aacy)/100 + abs(aacz)/100);
            
            // 显示阈值
            LCD_ShowNum(30, 16, alarm_threshold, 5, 16);
            
            if(alarm_threshold > 400)
            {
                // 进行预测
                prediction = predict(&svm_model, (float*)features, FEATURE_SIZE);
                LCD_ShowString(30, 0, 200, 16, 16, "BEEP ON ");
